...
int main()
{
    ...
    LOGI("%s", consoleTitle.c_str());
    std::cout << skCrypt("\n\n Connecting..");

    KeyAuthApp.init();
    if (!KeyAuthApp.response.success)
    {
        LOGE("%s", KeyAuthApp.response.message.c_str());
        sleep(1.5);
        return 1;
    }

    if (fileExists("test.json")) //change test.txt to the path of your file :smile:
    {
        if (!CheckIfJsonKeyExists("test.json", "username"))
        {
            ...
            LOGI("Successfully Automatically Logged In");
        }
        else
        {
    ...
    else
    {
        ...
        case 4:
            ...
            break;
        default:
            LOGE("Failure: Invalid Selection");
            sleep(3);
            return 1;
        }

        if (KeyAuthApp.response.message.empty()) return 11;
        if (!KeyAuthApp.response.success)
        {
            LOGE("%s", KeyAuthApp.response.message.c_str());
            sleep(1.5);
        ...
        if (username.empty() || password.empty())
        {
            ...
            LOGI("Successfully Created File For Auto Login");
        }
        else
        {
    ...
    */
    ...
    if (KeyAuthApp.user_data.username.empty()) return 10;
    std::cout << skCrypt("\n User data:");
...
void sessionStatus() {
    ...
    if (KeyAuthApp.response.isPaid) {
        while (true) {
            sleep(20); // this MUST be included or else you get blocked from API
            KeyAuthApp.check();
            if (!KeyAuthApp.response.success) {
                exit(0);
            }
